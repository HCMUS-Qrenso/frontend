  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Hiệu suất theo thời gian</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400">Theo dõi doanh thu và số order</p>
        </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            ...
            <YAxis
              ...
              tickFormatter={(value) => (dataType === "revenue" ? formatCurrency(value) : value.toString())}
              dx={-10}
            />
            <Tooltip
              contentStyle={{
      ...
      {/* Stats */}
      <div className="mt-6 grid gap-4 border-t border-slate-100 pt-6 dark:border-slate-800 sm:grid-cols-3">
        <div>
        ...
        <div>
          ...
          <p className="mt-1 text-xl font-semibold text-slate-900 dark:text-white">{avgOrdersPerDay}</p>
        </div>
      </div>
    </div>
  )
