...
export function MenuItemsOverviewStats() {
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
      <StatCard title="Tổng món" value="128" subtext="Đã tạo" icon={<UtensilsCrossed className="h-6 w-6" />} />
      <StatCard title="Đang bán" value="98" subtext="Available" icon={<CheckCircle2 className="h-6 w-6" />} />
      <StatCard title="Hết hàng" value="12" subtext="Sold out" icon={<XCircle className="h-6 w-6" />} />
      <StatCard title="Tạm ẩn" value="18" subtext="Unavailable" icon={<EyeOff className="h-6 w-6" />} />
      <StatCard title="Chef's picks" value="24" subtext="Recommendation" icon={<Award className="h-6 w-6" />} />
    </div>
  )
}
