...
import { useCurrentTenantQuery } from '@/hooks/use-tenants-query'
import type { MenuItem } from '@/types/menu-items'
import { generatePDFBlob } from './utils'
import { Template } from '@/types/menu-template'
...
export function TemplateExport({ templates, selectedTemplate }: TemplateExportProps) {
  ...
  // Fetch data
  const { data: categoriesData, isLoading: categoriesLoading, isError: categoriesError } = useCategoriesQuery({ status: 'active' })
  const { data: menuItemsData, isLoading: menuItemsLoading, isError: menuItemsError } = useMenuItemsQuery({
    status: 'available',
    limit: 100
  })
  const { data: tenantData, isLoading: tenantLoading, isError: tenantError } = useCurrentTenantQuery()
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid gap-6 lg:grid-cols-[400px_1fr] h-[75vh]">
            ...
            <Card className="rounded-2xl gap-0 p-0 border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 h-full overflow-hidden">
              {/* Tabs */}
                          ...
                          <div className="grid grid-cols-6 gap-2">
                            {['⭐', '👨‍🍳', '🔥', '❤️', '🌟', '👑', '🥇', '💎', '🎯', '🏆', '✨', '💫'].map((icon) => (
                              <button
                                ...
                                className={cn(
                                  'flex items-center justify-center p-2 rounded-lg border text-lg transition-colors',
                                  chefIcon === icon
            ...
            <div className="flex-1 flex flex-col">
              <Card className="flex-1 p-0 gap-0 rounded-2xl border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900 flex flex-col overflow-hidden">
                ...
                <div 
                  ref={canvasRef}
                  className={cn(
                    "flex-1 bg-slate-100 dark:bg-slate-800 pt-8 overflow-hidden min-h-150 select-none",
