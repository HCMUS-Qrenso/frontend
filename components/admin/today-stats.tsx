...
function StatCard({ title, value, subtext, trend, icon, className }: StatCardProps) {
  return (
    <div
      className={cn(
        "group rounded-2xl border border-slate-100 bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md dark:border-slate-800 dark:bg-slate-900",
        className,
      )}
    >
...
function OrderStatusCard() {
  ...
  return (
    <div className="group rounded-2xl border border-slate-100 bg-white p-5 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md dark:border-slate-800 dark:bg-slate-900">
      <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Trạng thái order</p>
      <p className="mt-2 text-3xl font-semibold tracking-tight text-slate-900 dark:text-white">{total}</p>
...
export function TodayStats() {
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
      ...
      />
      <StatCard title="Bàn đang phục vụ" value="12" subtext="8 bàn trống" icon={<Users className="h-6 w-6" />} />
      <OrderStatusCard />
      <StatCard
